...
M.pixi = function(cwd, subcommands, options, rest_args, extra_args)
  ...
  local pixi_path = nil
  local link_path = fs.join(store, 'pixis', name .. '.sym', 'content')
  local candidates = {
  ...
  local check_pixi = function(p)
    return p and fs.exists(p) and fs.exists(fs.join(p, 'pixi.toml'))
  end
  ...
  if pixi_path ~= path and pixi_path ~= link_path then
    if not fs.exists(link_path) then
      command = 'mkdir -p '
        .. fs.join(link_path, 'content')
        .. ' && '
        .. 'ln -sn '
        .. pixi_path
        .. ' '
        .. fs.join(link_path, 'link')
        .. ' && '
        .. 'ln -sn '
        .. fs.join(pixi_path, '*')
        .. ' '
  ...
  command = command .. 'pixi shell --manifest-path="' .. pixi_path .. '" '
  if env then
    command = command .. '-e ' .. env .. ' '
  end
  return confirm_command(command .. table.concat(extra_args, ' '), function()
