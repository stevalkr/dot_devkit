...
M.flake = function(cwd, subcommands, options, rest_args, extra_args)
  ...
  local candidates = { (options['name'] or options['path']) and path or false, cwd, fs.join(cwd, 'flake'), path,
    link_path }
  local check_flake = function(p) return p and fs.exists(p) and fs.exists(fs.join(p, 'flake.nix')) end
  ...
  command = command ..
      'nix develop "' .. flake_path .. '" ' .. save .. ' ' .. table.concat(extra_args, ' ') .. ' --command ' .. cmd
